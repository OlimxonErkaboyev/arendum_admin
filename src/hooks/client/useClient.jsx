...
const useClient = create(
  devtools((set) => ({
    ...
    getList: async (params) => {
      ...
      try {
        const { data } = await requests.fetchClientList(params);
        set({
          clients: data?.data,
    ...
    getDetail: async (id) => {
      ...
      const { data } = await requests.fetchClientDetail(id);
      set({
        detail: data,
        detailLoading: false,
      });
    ...
    update: async (id, params) => {
      ...
      try {
        const data = await requests.clientUpdate(id, params);
        return data;
      } catch ({ response }) {
        return response;
      } finally {
    ...
    remove: async (id) => {
      ...
      try {
        const { data } = await requests.clientDelete(id);
